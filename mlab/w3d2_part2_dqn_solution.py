...
class QNetwork(nn.Module):
    def __init__(
        ...
        current = hidden_sizes[0]
        for i in range(1, len(hidden_sizes)):
            layers.append(nn.Linear(current, hidden_sizes[i]))
            layers.append(nn.ReLU())
            current = hidden_sizes[i]
        layers.append(nn.Linear(current, num_actions))
...
if MAIN:
    ...
    if "SKIP":
        if False:  # Don't want to see these while debugging train loop
            import pandas as pd
...
def linear_schedule(
    ...
    """
    "SOLUTION"
    duration = int(exploration_fraction * total_timesteps)
    assert duration > 1
    slope = (end_e - start_e) / (duration - 1)
...
if MAIN:
    ...
    if "SOLUTION":
        if False:  # Don't want to see while debugging train loop
            fig, ax = plt.subplots()
            ax.plot(epsilons)
            ax.set(xlabel="Step", ylabel="Epsilon")
            fig.suptitle("Probability of Random Action")
            if "SKIP":
                fig.savefig("./w3d2_epsilon_schedule.png", facecolor="#FFF")
