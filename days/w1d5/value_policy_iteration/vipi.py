...
def policy_evaluation(P, R, policy, gamma=0.9, tol=1e-2):
    ...
    K = int(Ns*Ns*np.log(tol)/np.log(gamma))

    for k in range(1000):
        #print(k, K)
        for s in range(Ns):
            
            res = sum([P[s_, policy[s], s] * value_function[s_] for s_ in range(Ns)])
            new_value_function[s] = R[s, policy[s]] + gamma * res
        #if np.max(np.abs(value_function-new_value_function)) < tol/10:
         #   break
